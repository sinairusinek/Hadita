...
PAGE_FOLIO = {3: "1", 10: "9", 33: "32", 34: "33", 35: "34", 50: "49"}
...
def load_gemini3(page_num: int) -> list[dict]:
    """Load cached Gemini-3 showcase OCR results.

    Generated by: python run_showcase_pages.py {N}
    Cache file:   .ocr_cache/G3_page{N}.json
    """
    cache_file = CACHE_DIR / f"G3_page{page_num}.json"
    if cache_file.exists():
        return json.loads(cache_file.read_text())
    return []
...
OCR_SOURCES = {
    ...
    "Gemini 3 (showcase)":             load_gemini3,
    ...
    if "cv_all_rows" not in st.session_state:
        ...
        for p in PAGES:
            if p in gt_by_page:
                ...
                ocr_rows = _load_ocr(p) or load_gemini3(p)
                for r in ocr_rows:
                    row = dict(r)
                    row["_page"] = p
                    combined.append(row)
